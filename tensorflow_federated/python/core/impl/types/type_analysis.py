...
def is_binary_op_with_upcast_compatible_pair(
  ...
  if possibly_nested_type.is_equivalent_to(type_to_upcast):
    return True
  if (
      not isinstance(type_to_upcast, computation_types.TensorType)
      or type_to_upcast.shape is None
      or type_to_upcast.shape
  ):
    return False
...
def check_is_sum_compatible(type_spec, type_spec_context=None):
  ...
  if isinstance(type_spec, computation_types.TensorType):
    ...
    if not array_shape.is_fully_defined(type_spec.shape):
      raise SumIncompatibleError(
          type_spec,
          type_spec_context,
...
def is_single_integer_or_matches_structure(
  ...
  if isinstance(type_sig, computation_types.TensorType):
    ...
    return type_sig.dtype.is_integer and (
        array_shape.num_elements(type_sig.shape) == 1
    )
...
def count_tensors_in_type(
  ...
  def _capture_tensors(type_signature):
    if isinstance(
      ...
      num_parameters = array_shape.num_elements(type_signature.shape)
